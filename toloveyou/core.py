...
DB = '/home/orange/000/.toloveyou/toloveyou.db'
...
def init_db(cursor=None):
    cursor.execute('''
        create table if not exists toloveyou (
            entry text not null,
...
def save_attr(entry, attr, value, cursor=None):
    ...
    cursor.execute('''
        INSERT INTO toloveyou (entry, attribute, value)
        VALUES (?, ?, ?)
